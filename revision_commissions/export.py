...
def get_formula(x, y, i):
    ...
    '''
    add_thousand = [1, 2, 4, 5, 6, 7]
    if i in add_thousand:
        x *= 1000
        y *= 1000
    '''
    if i == 1:
...
def get_score(num, i):
    ...
    if i == 1:
        return 3 if num >= 30 else 2 if num >= 20 else 1 if num >= 15 or num <= 15 else 0
    if i == 2:
        return 3 if num < 93 else 2 if num < 97 else 1 if num < 100 else 0
    if i == 3:
        return 3 if num >= 3 else 2 if num >= 2 else 1 if num >= 1 or num <= 1 else 0
    if i == 4:
        return 3 if num <= 80 else 2 if num <= 90 else 1 if num <= 100 else 0
    if i == 5:
        return 3 if num >= 10 else 2 if num >= 5 else 1 if num >= 0 else 0
    if i == 6:
...
titles = ["Годовой объем финансирования учреждения",
        "Рост/снижение объемов финансирования по сравнению с прошлым периодом",
        ...
        "Доля финансовых нарушений от общего объема расходов текущего периода",
...
'''
def export_organization(arr, filename):
    workbook = xlsxwriter.Workbook(f'{BASE_DIR}/revision_commissions/exports/{filename}.xlsx')
    ...
    for i in range(2):
        ...
        # Название
        worksheet.write(i, col, titles[i])
        col += 1

        # У первой строки нет формулы
        if i==0 or i==8:
            number = arr[i]
            worksheet.write(i, col, str(number)+metrics[i])
        else:
            #print(f"data ind: {data_ind}")
        ...
        # баллы
        worksheet.write(i, col, get_score(number, i))
...
def generate_organization_data(arr):
    result = []
    data_ind = 0
    for i in range(15):
        row = []
...
if __name__=="__main__":
    ...
    org = generate_organization_data(fake_data)
    print(org)
